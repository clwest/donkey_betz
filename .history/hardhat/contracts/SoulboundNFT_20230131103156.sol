...
contract SoulboundNFT is ERC721, ERC721URIStorage {
    GrantUserRoles public roles;
    // Globals
    // The address of the contract owner
    address public owner;

    // TotalSupply: The total number of NFTs that have be minted
    ...
    mapping(address => mapping(uint256 => bytes32)) public tokenURIMapping;
    // metadata: A mapping of Token IDs to the metadata URI
    mapping(uint256 => nftMetadataMapping) public nftMetadataMapping;
    ...
    struct User {
        ...
        uint256 personalAssistant; // holds nftMetadataMapping struct
        uint256 nftMetadataMapping; // mapping to the NFT Metadata struct
        bytes32 ipfsHash;
    ...
    modifier onlyOwnerAndMinter() {
        require(
            isOwner() || hasRole(roles[roles.MINTER_ROLE], msg.sender),
    ...
    modifier canNotTransfer(uint256 _tokenId) {
        require(userData[_tokenId] == msg.sender, "This token is SOUL BOUND.");
        _;
    }
    ...
    function mintNFT(
        ...
        require(roles.isMinter(), "Sender must have minter role");
        require(tokensOwned[msg.sender] == 0, "Address already owns an NFT");
    ...
    function mintSoulboundNFT(
        ...
        require(roles.hasRole(msg.sender, roles.ADMIN_ROLE));
        ...
        roles.grantRole(_to, roles.MINTER_ROLE);
        // Check that the owner does not already own an NFT
        require(tokensOwned[_to] == 0, "User already owns a unique NFT");
    ...
    function modify(
        ...
        nftMetadataMapping[_tokenId].name = _name;
        nftMetadataMapping[_tokenId].description = _description;
        nftMetadataMapping[_tokenId].image = _image;
        nftMetadataMapping[_tokenId].tags = _tags;
        nftMetadataMapping[_tokenId].uri = _uri;
    }
    ...
    function balanceOf(address _owner) public view returns (uint256) {
        ...
        return tokensOwned[_owner] == 0 ? 0 : 1;
    }

    // function function to set the tokenURIMapping for the soulbound NFT

    // checks if the msg.sender has the role
    function hasRole(bytes32 role, address account) public view returns (bool) {
        return roles.hasRole(role, account);
    }
