...
contract GrantUserRoles is AccessControl {
    ...
    // Create an instance of SoulboundNFT
    SoulboundNFT private SoulboundNFT;
    ...
    mapping(address => mapping(uint256 => bool)) public tokensOwned;
    ...
    constructor() {
        roleBytes[Roles.MINTER_ROLE] = keccak256("MINTER_ROLE");
        roleBytes[Roles.ADMIN_ROLE] = keccak256("ADMIN_ROLE");
    ...
    function revokeRole(address user, bytes32 role) public {
        require(
            role == roleBytes[Roles.MINTER_ROLE] ||
                role == roleBytes[Roles.ADMIN_ROLE],
            "Invalid role"
        );
        _revokeRole(role, user);

        if (role == roleBytes[Roles.MINTER_ROLE]) {
            minters[user] = false;
        }
    ...
    function hasRole(
        ...
        require(
            roleBytes[Roles.MINTER_ROLE] == role ||
                roleBytes[Roles.ADMIN_ROLE] == role,
            "Invalid role"
        );
    ...
    // Allows users to request a new role or be suggested for a new role
    function requestRoleAssignment(address _user) public {
